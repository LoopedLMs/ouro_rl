    ...
    def update(
        ...
        if cached_key is None:
            ...
            if (
                ...
                raise ValueError("Cached and incoming key/value tensors must match on batch, head, and head_dim dimensions.")
...
class OuroModel(OuroPreTrainedModel):
    def __init__(self, config: OuroConfig):
        ...
        self.layers = nn.ModuleList([OuroDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
